...
from traclus_impl.trajectory_partitioning import NEW_partition_trajectory_caller
from traclus_impl.trajectory_partitioning import NEW_partition_trajectory_caller
...
class SimpleLinePartitioningIntegTest(UnitBaseTests):
    ...
    def test_simple_line_segment(self):
        ...
        self.verify_iterable_works_more_than_once(expected_par, \
                                                  NEW_partition_trajectory_caller(trajectory_point_list=trajectory))
    ...
    def test_longer_obvious_NOT_partition(self):
        ...
        self.verify_iterable_works_more_than_once(NEW_partition_trajectory_caller(trajectory_point_list=trajectory), 
                                                  expected_par)
    ...
    def test_not_enough_points(self):
        self.assertRaises(ValueError, NEW_partition_trajectory_caller, [Point(1, 1)])
        self.assertRaises(ValueError, NEW_partition_trajectory_caller, [])
        
    def test_three_points_horizontal(self):
        traj = [Point(0, 0), Point(1.6, 0), Point(4.4, 0)]
        expected_par = [0, 2]
        self.verify_iterable_works_more_than_once(NEW_partition_trajectory_caller(trajectory_point_list=traj), \
                                                  expected_par)  
